<template>
  ...
  <div v-if="addF" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
    <!-- 卡片容器 -->
    <div class="bg-white rounded-lg shadow-lg p-5 w-96 mx-6 relative">
      ...
      <form @submit.prevent="joinFmaily(familyCode)" class="space-y-4 ">
        ...
        <input class="border-2 w-full p-2 rounded-md outline-none" type="text " v-model="familyCode"
          placeholder="輸入家庭代碼" />
        <button class="w-full bg-[#92C700] text-white py-2 rounded-md" type="submit">
          手動加入
        </button>
      </form>
      <!-- 分隔線 -->
      ...
      <div class="flex justify-center">
        <button class="bg-[#73AA00] text-white py-2 px-4 rounded-md hover:bg-blue-600" @click="openCamera">
          掃描QR code
        </button>
      </div>
    </div>
        ...
        <template v-slot:open-slot>
          <div class="flex items-center flex-row ">
            ...
            <div
              ...
              <PhFolderPlus :size="28" weight="thin" />
            </div>
          </div>
        </template>
