<template>
  ...
  <form @submit.prevent="sendVerificationCode(verificationCode)" class="flex flex-col space-y-10 px-10 mt-3">
    ...
    <div class="flex border-2  rounded-md shadow-md p-1 px-3 justify-between">
      ...
      <input class="border-none resize outline-none p-1 mx-1 w-full" type="password" v-model="verificationCode"
        placeholder="驗證碼" />
    ...
    <button type="submit"
      class=" bg-[#90c700] rounded shadow-md items-center text-white font-bold text-H3 p-2">繼續</button>
  </form>
...
const startCountdown = async () => {
  ...
  // 禁用按鈕並開始倒計時
  try {
    await request.get("/verify", { params: { email: email } })
    showMsg("已發送驗證碼")
  } catch (err: any) {
    if (isAxiosError(err)) {
